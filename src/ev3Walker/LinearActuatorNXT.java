...
import lejos.hardware.device.MMXMotor;
...
public class LinearActuatorNXT implements LinearActuator {
	// private static final int MIN_POWER = 30;

	private EncoderMotor encoderMotor;
	private volatile int motorPower = 0;
	private volatile int tick_wait; // this is calculated in setPower() to fit
									// the power setting. Variable because lower
									// powers move it slower.
	...
	//
	/**
	 * Create a <code>LnrActrFirgelliNXT</code> instance. Use this constructor
	 * to assign an instance of <code>EncoderMotor</code> used to drive the
	 * actuater motor. This constructor allows any motor class that implements
	 * the <code>EncoderMotor</code> interface to drive the actuator. You must
	 * instantiate the <code>EncoderMotor</code>-type motor before passing it to
	 * this constructor.
	 * <p>
	 * When an instance of the MMXMotor class is used, the motor power curve is
	 * much different that with a NXTMotor (due to a different PWM output?) .
	 * The speed peaks out at about 20% power so in effect, the speed control
	 * granularity is coarser.
	 * <p>
	 * The default power at instantiation is 100%.
	 * 
	 * @param encoderMotor
	 *            A motor instance of type <code>EncoderMotor</code> which will
	 *            drive the actuator
	 * @see lejos.hardware.motor.NXTMotor
	 * @see MMXMotor
	 * @see EncoderMotor
	 */
	public LinearActuatorNXT(EncoderMotor encoderMotor) {
	 ...
	 * <code>NXTMotor</code> using the specified motor port. This instance is
	 * then used to drive the actuator motor.
	 * <p>
	 * The default power at instantiation is 100%.
	 * 
	 * @param port
	 *            The motor port that the linear actuator is attached to.
	 * @see lejos.hardware.port.MotorPort
	 * @see NXTMotor
	 */
	public LinearActuatorNXT(Port port) {
		this(new NXTMotor(port));
	}

	/**
	 * Sets the power for the actuator. This is called before the
	 * <code>move()</code> or <code>moveTo()</code> method is called to set the
	 * power. Using lower power values and pushing/pulling an excessive load may
	 * cause a stall and in this case, stall detection will stop the current
	...
	// }
	//
	// /**Returns true if a <code>move()</code> or <code>moveTo()</code> order
	// ended due to a motor stall. This behaves
	// * like a latch where the
	// * reset of the stall status is done on a new <code>move()</code> or
	// <code>moveTo()</code> order.
	// * @return <code>true</code> if actuator motor stalled during a movement
	...
	/**
	 * Causes the actuator to move <code>distance</code> in encoder ticks. The
	 * <code>distance</code> is relative to the actuator shaft position at the
	 * time of calling this method. Positive values extend the actuator shaft
	 * while negative values retract it. The Firgelli L12-NXT-50 & 100 use 0.5
	 * mm/encoder tick. eg: 200 ticks=100 mm.
	 * <p>
	 * Stall detection stops the actuator in the event of a stall condition to
	 * help prevent damage to the actuator.
	 * <P>
	 * If <code>immediateReturn</code> is <code>true</code>, this method returns
	 * immediately (does not block) and the actuator stops when the stroke
	 * <code>distance</code> is met [or a stall is detected]. If another
	 * <code>move</code> action is called before the stroke distance is reached,
	 * the current actuator action is cancelled and the new action is initiated.
	 * <p>
	 * If the stroke <code>distance</code> specified exceeds the maximum stroke
	 * length (fully extended or retracted against an end stop), stall detection
	 * will stop the action. It is advisable not to extend or retract to the
	 * stop as this is hard on the actuator. If you must go all the way to an
	 * end stop and rely on stall detection to stop the action, use a lower
	 * power setting.
	 * 
	 * @param distance
	 *            The Stroke distance in encoder ticks.
	 * @param immediateReturn
	 *            Set to <code>true</code> to cause the method to immediately
	 *            return while the action is executed in the background.
	 *            <code>false</code> will block until the action is completed,
	 *            whether successfully or stalled.
	 * @see #setPower
	 * @see #stop
	 * @see #getTachoCount
	...
	public void move(int distance, boolean immediateReturn) {
		...
		while (!LinearActuatorNXT.this.killCurrentAction) {
			...
			if (LinearActuatorNXT.this.distanceTicks - tacho <= 4 && power > 80)
				LinearActuatorNXT.this.encoderMotor.setPower(70);
			// exit loop if destination is reached
			if (tacho >= LinearActuatorNXT.this.distanceTicks)
				break;
			// if power changed during this run.... (only when
			...
			if (LinearActuatorNXT.this.killCurrentAction)
				break;
			doWait(LinearActuatorNXT.this.tick_wait / 2);
		}
	...
	/**
	 * Causes the actuator to move to absolute <code>position</code> in encoder
	 * ticks. The <code>position</code> of the actuator shaft on startup or when
	 * set by <code>resetTachoCount()</code> is zero.
	 * 
	 * @param position
	 *            The absolute shaft position in encoder ticks.
	 * @param immediateReturn
	 *            Set to <code>true</code> to cause the method to immediately
	 *            return while the action is executed in the background.
	...
	// // If we already have an active command, signal it to cease and wait
	// until cleared
	...
	// }
	//
	// // if told to block, wait until the actuator thread completes its current
	// task. When done, it will do a notify() to wake
	// // us up here.
	// if (!immediateReturn) {
	// synchronized(this.synchObj1){
	 ...
	 * The zero position of the actuator shaft is where
	 * <code>resetTachoCount()</code> was last called or the position of the
	 * shaft when instantiated.
	 * <p>
	 * The Firgelli L12-NXT-50 & 100 use 0.5 mm/encoder tick. eg: 200 ticks=100
	 * mm.
	 * 
	...
	/**
	 * Resets the tachometer (encoder) count to zero at the current actuator
	 * shaft position.
	 * 
	 * @see #getTachoCount
	 */
